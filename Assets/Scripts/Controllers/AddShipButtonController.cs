...
public class AddShipButtonController : EventTrigger
{
    public override void OnPointerDown(PointerEventData eventData)
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            c.ColonizableManager is IColonizableManager m && Validate(m.CurrentColony))
                m.CurrentColony.AddRocket();
        }
    }

    private bool Validate(Colony colony)
    {
        return colony.Goods.ContainsKey(EGood.Hydrogen) && colony.Goods[EGood.Hydrogen].Value >= 100
